## SubWallet Extension
Our SubWallet is forked from polkadot-js/extension. We aim to add more features while being able to rebase the polkadot-js origin at any time.
## Main Concept
...
- Store class should extend class `BaseStore` or `SubscribableStore` and overwrite class prefix call via constructor.
  ...
  - `SubscribableStore` extend `BaseStore`, include rxjs subject and can be subscribed with method `getSubject()`. This subject triggers every time data is set.
  ``` typescript
...
Subwallet extension uses message passing concept via browser API to interact between Background - Extensions - Chrome Tabs.
...
- There are 2 message type:
  ...
  - Subscription message: Same as one time message but listener continue receive data until window close.
